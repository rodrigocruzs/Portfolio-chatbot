...
const account_list = document.getElementById('account-list');
...
const gpt_image = `<img src="/client/img/gregai.png" alt="Greg Avatar">`;
...
const cacheConversationMemory = async (conversation_id, memory) => {

};

const displayConnectedAccounts = async () => {
    try {
        ...
        // Create a list element for each account and append it to the account list
        accounts.forEach(account => {
            const listItem = document.createElement('li');
            listItem.textContent = account;
            account_list.appendChild(listItem);
        });
...
window.addEventListener('load', displayConnectedAccounts);
...
const ask_gpt = async (message) => {
    try {
        ...
        `;
		const maxMessageHistory = 4;  // number of last messages to remember
		const fullConversationHistory = await get_conversation(window.conversation_id);
		const recentConversationHistory = fullConversationHistory.slice(-maxMessageHistory);
        ...
        const response = await fetch('/api/conversation', {
            ...
            body: JSON.stringify({
                input: message,
				messagehistory: recentConversationHistory,
        ...
        if (e.name != `AbortError`) {
            let error_message = `Oops! Something went wrong. Please create a new conversation`;
